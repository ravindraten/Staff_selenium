	 ...
	 public void openBrowser(String browserName) throws InterruptedException, MalformedURLException{
		 ...
		 try {
				if (browserName.equalsIgnoreCase("firefox")) {
					...
					driver = new RemoteWebDriver(new URL("http://localhost:4444/wd/hub"), optionsF);
				    driver.manage().deleteAllCookies();
				    driver.get("http://127.0.0.1:8080");
				    driver.manage().window().maximize();
				    driver.manage().timeouts().implicitlyWait(4, TimeUnit.SECONDS);
				} else if (browserName.equalsIgnoreCase("chrome")) {
					System.setProperty("webdriver.chrome.driver","\\Users\\ravindra\\Downloads\\chromedriver");
